...
export class UIManager {
	...
	static handleStartScreenFocusGuard(e, gameInfo, startBtn) {
		...
		const targetIsStart = t === startBtn || (t && 'closest' in t && t.closest && t.closest('#startBtn'));
		if (!targetIsStart) {
	...
	static handleGameOverFocusGuard(e, gameOverScreen, restartBtn) {
		...
		const t = /** @type {Element|null} */(e && e.target instanceof Element ? e.target : null);
		const targetIsRestart = t === restartBtn || (t && 'closest' in t && t.closest && t.closest('#restartBtn'));
		if (!targetIsRestart) {
			if (e.cancelable) e.preventDefault();
			e.stopPropagation();
		}
