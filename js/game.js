...
import { updateAsteroids, updateBullets, updateEngineTrail, updateExplosions, updateParticles, updateStars } from './systems/UpdateSystems.js';
...
class DarkHorizon {
    ...
    this.events = new EventBus();
        ...
        this.particlePool = new ObjectPool(
            (x, y, vx, vy, life, maxLife, size, color) => new Particle(x, y, vx, vy, life, maxLife, size, color),
            undefined,
            { maxSize: 4096 }
        );
    ...
    this._setupEventSubscriptions();
        ...
        this.loop = new GameLoop({
            ...
            stepMs: 1000 * CONFIG.TIME.DEFAULT_DT,
            maxSubSteps: 5,
        });
    }
        ...
        this.events.on('collectedStar', ({ star }) => {
            ...
            for (let p = 0; p < CONFIG.STAR.PARTICLE_BURST; p++) {
                this.particles.push(this.particlePool.acquire(
                    ...
                    Math.cos((Math.PI * 2 * p) / CONFIG.STAR.PARTICLE_BURST) * (rng.range(0, 180) + 120),
                    Math.sin((Math.PI * 2 * p) / CONFIG.STAR.PARTICLE_BURST) * (rng.range(0, 180) + 120),
    ...
    shouldTogglePause(e) {
        ...
        const isPauseKey = CONFIG.INPUT.PAUSE_CODES.includes(e.code) || CONFIG.INPUT.PAUSE_KEYS.includes(e.key);
     ...
     */
    ensureOverlayFocus() {
        UIManager.ensureOverlayFocus(this.gameInfo, this.startBtn, this.gameOverScreen, this.restartBtn);
    }
    ...
    handleVisibilityChange() {
        UIManager.handleVisibilityChange(this.gameInfo, this.startBtn, this.gameOverScreen, this.restartBtn);
    }
     ...
     */
    resetGameState() {
        const releaseAll = (arr, pool) => { if (!arr || !pool) return; for (const it of arr) pool.release(it); };
    ...
    createBullet() {
        ...
        return this.bulletPool.acquire(bx, this.player.y, CONFIG.BULLET.WIDTH, CONFIG.BULLET.HEIGHT, this.bulletSpeed);
    }
    ...
    createExplosion(x, y) {
        ...
        for (let i = 0; i < CONFIG.EXPLOSION.PARTICLE_COUNT; i++) {
            const vx = (rng.nextFloat() - 0.5) * 480;
            const vy = (rng.nextFloat() - 0.5) * 480;
            const size = rng.range(0, CONFIG.EXPLOSION.PARTICLE_SIZE_VARIATION) + CONFIG.EXPLOSION.PARTICLE_SIZE_MIN;
        ...
        this.explosions.push(this.explosionPool.acquire(
